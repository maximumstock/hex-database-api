  ...
  findAndParse: function startUpdate() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      request(options, function(err, res, body) {
        ...
        // grab HTML table headers
        const headers = [];
        $('div.CSSTableGenerator tr:first-child td').each(function(i, elem) {
          headers.push($(this).text());
        });

        // grab the actual HTML rows with item information
        const rows = [];
        $('div.CSSTableGenerator tr:first-child').nextAll().each(function(i, elem) {
          ...
          $(this).find('td').each(function(i, elem) {
            if ($(this).children().length !== 0) {
              let html = $(this).html();
              // remove `<p></p>` instances that were meant to create line breaks; usually is the case in
              // description texts of cards
        ...
        rows.forEach(function(r) {
          ...
          for (let i = 0; i < r.length; i++) {
            ...
            // if the currently old header value is in `HEADER_MAP`, then map it
            if (HEADER_MAP.hasOwnProperty(headers[i])) {
              propertyName = HEADER_MAP[headers[i]].name;

              // check `HEADER_MAP` if the new header value needs to be casted
              ...
              /* ------------------------------------------------------------- */
              // The following three blocks describe special parsing behaviour for specific
              // header values. All of these should be stored within arrays, so we have to do
              // some further transforming.
              ...
              if (headers[i] === 'THRESHOLD') {
                ...
                if (r[i].length === 0) {
                  ...
                  // also set color to Colorless
                  record.color = ['Colorless'];
                } else {
                  ...
                  record.color = thresholds.map(function(t) {
                    return t.shard
                  }).sort();
                }
              ...
              // COLORS ARE NOW PARSED WITH THRESHOLDS (ABOVE)
              ...
              // special equipment uuid parsing
              if (headers[i] === 'EQUIPMENT UUIDS') {
                r[i] = r[i].trim().split(',').map(function(euuid) {
                  return euuid.trim();
                }).filter(function(euuid) {
                  // filter out all euuids that are not valids ids (aka are too short
                  return euuid.length >= 32;
                }).sort();

                if (r[i].length === 0) {
                  r[i] = []; // set value for `equipment_uuids` to an empty array if there are none (aka it's not a card?!)
                }

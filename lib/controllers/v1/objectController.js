...
module.exports = {
  ...
  /**
   * @api {post} /objects/search Retrieve a list of objects that meet given criteria
   */
  search: function* search(next) {
    ...
    // `uuid`
    if (this.request.body.uuid && typeof this.request.body.uuid === 'string') {
      query.whereRaw('data->>\'uuid\' = ?', [this.request.body.uuid]);
    }

    // `artist`
    if (this.request.body.artist && typeof this.request.body.artist === 'string') {
        query.whereRaw('data->>\'artist\' = ?', [this.request.body.artist]);
    }
  ...
  /**
   * @api {get} /objects/properties/:propertyName Retrieve a list of distinct values for :propertyName
   */
